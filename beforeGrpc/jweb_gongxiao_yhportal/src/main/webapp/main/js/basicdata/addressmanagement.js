$(function () {
    ...
    function audit(URL, row) {
        ...
        $.get(URL + "/addressAudit/auditAddress", {"addressId": addressId}, function (res, status, xhr) {
            if (res.returnCode === 0) {
                $("#yhtable").bootstrapTable("refreshOptions", {pageNumber: 1});
            }
        }, "json")
...
function reject(URL, row) {
    var addressId = row.addressId;
    $.get(URL + "/addressAudit/rejectAddress", {"addressId": addressId}, function (res, status, xhr) {
        if (res.returnCode === 0) {
            $("#yhtable").bootstrapTable("refreshOptions", {pageNumber: 1});
        }
    }, "json")
...
function renderTable(URL) {
    $("#yhtable").bootstrapTable({
            ...
            url: URL + "/addressAudit/selectNeedAuditAddressList",
            queryParamsType: '',
            silent: true,
            undefinedText: '',
            striped: true,
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    if (!row) {
                        ...
                        if (row.auditStatus == 1) {
                            return [
                                '<a class="audit" href="javascript:void(0)">', '审核 ', '</a>',
                                '<a class="refuse" href="javascript:void(0)">', ' 驳回', '</a>'
                            ].join('');
                        }
                    }
                },
                ...
                formatter: function (envet, value, row, index) {
