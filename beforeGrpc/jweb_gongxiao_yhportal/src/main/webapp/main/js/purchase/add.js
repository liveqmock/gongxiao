    ...
    function bindBackEvent(URL) {
        ...
        $purchaseSubmit.click(function () {
            ...
            var orderParams = {
                ...
                supplierCode: $("#supplierlist").val(),
                //付款方式
                paymentMode: $("#paidstyle").val(),
                //物流方式
                logisticType: $("#logistic").val(),
                //业务发生日期
                businessDate: $("#yhbusiness-date").val(),
                //要求到货日期
                requirArrivalDate: $("#yharrival-date").val(),
                //到货截止日期
                ...
                //采购总数数量
                purchaseNumber: $("#pruchase-totalnum").text(),
                //返利使用金额
                couponAmountUse: $couponAmont.val(),
                //代垫使用金额
                prepaidAmountUse: $purchasePrepaid.val(),
                //返利过账金额
                adCouponAmountUse : $couponPostAmount.val(),
                //代垫过账金额
                adPrepaidAmountUse : $prepaidPostAmount.val(),
                //采购指导金额
                purchaseGuideAmount: $("#purchase-guideamount").text(),
                //采购优惠金额
                purchasePrivilegeAmount: $("#purchase-privilege").text(),
                //现在返点金额
                returnCash: $("#purchase-returncash").text(),
                //采购应付金额
                purchaseShouldPayAmount: $("#purchase-shouldpay").text(),
                //采购实付金额
                purchaseActualPayAmount: $("#purchase-pay").text(),
                //采购货品列表jsonArray
           ...
           /* verification.noEmpty("yharrival-date","VerificationEmpty6","输入的字符不能为空");
            ...
            /*if(purchaseTableData && ($couponAmont.val() || $purchasePrepaid.val() || $couponPostAmount.val() || $prepaidPostAmount.val())){
                $.post(URL + "/purchase/order/generatePurchaseOrders", orderParams, function (res) {
                    if (res.returnCode === 0) {
                        layui.config({
                            version: '1515376178709' //为了更新 js 缓存
                        });
                ...
                if(!purchaseTableData){
                    $.renderLayerMessage("请先添加商品！",2);
                }else{
                    $.renderLayerMessage("请填写返利金额！",2);
                }
            }*/
        })
    }
            ...
            columns: [{
                ...
                formatter: function (value, row) {
                    if (!value) {
                        ...
                        return '<input type="text" class="yhpurinput discountinput" value="' + value + '"' + '/>';
                    } else {
                        row.purchaseDiscount = value;
                        return '<input type="text" class="yhpurinput discountinput" value="' + value + '"' + '/>';
                    }
                },
                events: purDiscountEvents = {
                    'blur .discountinput': function (event, value, row, index) {
                        var $val = $(this).val();
                        $purchaseAddTable.bootstrapTable('updateCell', {
                            index: index,
                            field: 'purchaseDiscount',
                        ...
                        // 计算货品价格
                        $.post(URL + "/purchase/order/calculateProductInfo", calcPriceParams, function (res, status, xhr) {
                            if (res.returnCode === 0) {
                                var resData = res.data;
                                var calculateProductInfoList = resData.calculateProductInfoList;
                                // 使用返利 couponAmount 使用代垫 prepaidAmount 采购单价 purchasePrice 成本价  couponBasePrice 返利返利单价基数 purchaseDiscount
