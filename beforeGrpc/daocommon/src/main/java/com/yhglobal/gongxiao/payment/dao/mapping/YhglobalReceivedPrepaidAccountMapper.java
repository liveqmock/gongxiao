...
public interface YhglobalReceivedPrepaidAccountMapper extends BaseMapper {

    @Insert({
            "insert into yhglobal_received_prepaid_account (rowId, projectId, ",
    ...
    int insert(YhglobalReceivedPrepaidAccount record);

    @Select({
            "select",
            "rowId, projectId, projectName, supplierId, supplierName, status, totalAmount, ",
            "dataVersion, createTime, lastUpdateTime, tracelog",
            "from yhglobal_received_prepaid_account",
            "where projectId = #{projectId}"
    })
    YhglobalReceivedPrepaidAccount getAccount(@Param("projectId") long projectId);

    @Update({
            "update yhglobal_received_prepaid_account",
    ...
    int updateByPrimaryKeyWithBLOBs(YhglobalReceivedPrepaidAccount record);
}