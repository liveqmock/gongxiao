<template>
  <div class="supplier-frozen-account" id="supplier-frozen-account">
      ...
      <ul class="clear-float">
        <li v-for="(discountItem,index) in discount" :key="discountItem.accountType">
          ...
          <p>
            ...
            <a @click="switchDialog(discountItem.title,discountItem.accountType)">转<br/>出</a>
          </p>
        </li>
      </ul>
    </div>
    ...
    <div class="supplier-frozen-account-flow-table">
  ...
  export default {
    ...
    data() {
      return {
        ...
        accountType: [{accountCode: 1, label: '单价折扣'}, {accountCode: 2, label: '采购预留'}],
        accountFlow: [{flowNo: 306, label: '收入'}, {flowNo: 305, label: '支出'}],
        discount: [{title: "单价折扣", accountType: 1, discountNumber: 0}, {title: "采购预留", accountType: 2, discountNumber: 0}],
    ...
    methods: {
      ...
      getAccountAmount() {
        ...
        this.$http.get('/supplier/frozen/accountAmount', {
          params: amountParams,
        }).then((res) => {
          let {data: {data, message, returnCode}} = res;
          if (returnCode === 0) {
            this.discount[0].discountNumber = data['frozenDiscountAccountStr'];
            this.discount[1].discountNumber = data['frozenPurchaseAccountStr'];
          } else {
            console.log(message)
          }
        }).catch(function (err) {
    ...
    activated() {
      eventBus.$on('loadingDefaultProjectId', () => {
        ...
        if (!this.changedProjectId || !this.tableData.length) {
          this.getTableData();
          this.getAccountAmount();
        }
      });
