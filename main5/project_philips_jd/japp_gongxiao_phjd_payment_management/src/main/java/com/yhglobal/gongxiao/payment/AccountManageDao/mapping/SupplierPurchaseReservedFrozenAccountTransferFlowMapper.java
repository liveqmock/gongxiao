...
public interface SupplierPurchaseReservedFrozenAccountTransferFlowMapper extends BaseMapper{
    @Delete({
        "delete from supplier_purchase_reserved_frozen_account_transfer_flow",
        "where flowId = #{flowId,jdbcType=BIGINT}"
    })
    int deleteByPrimaryKey(Long flowId);

    @SelectProvider(type = SupplierPurchaseReservedFrozenAccountTransferFlowSqlProvider.class, method = "selectByConditions")
    List<SupplierPurchaseReservedFrozenAccountTransferFlow> selectByConditions(@Param("moneyFlow") Integer moneyFlow,
                                                                               @Param("dateS") Date dateS,
                                                                               @Param("dateE") Date dateE, @Param("tablePrefix") String tablePrefix);
    @SelectProvider(type = SupplierPurchaseReservedFrozenAccountTransferFlowSqlProvider.class, method = "selectTotal")
    List<FlowSubtotal> selectTotal(@Param("moneyFlow") Integer moneyFlow,
                                   @Param("dateS") Date dateS,
                                   @Param("dateE") Date dateE, @Param("tablePrefix") String tablePrefix);
    @Insert({
        "insert into ${tablePrefix}_supplier_purchase_reserved_frozen_account_transfer_flow (flowId, flowNo, ",
    ...
    int insert(SupplierPurchaseReservedFrozenAccountTransferFlow record);
    ...
    @Select({
        ...
        "from supplier_purchase_reserved_frozen_account_transfer_flow",
        "where flowId = #{flowId,jdbcType=BIGINT}"
    })
    @Results({
    ...
    SupplierPurchaseReservedFrozenAccountTransferFlow selectByPrimaryKey(Long flowId);



    @Update({
        "update supplier_purchase_reserved_frozen_account_transfer_flow",
    ...
    int updateByPrimaryKey(SupplierPurchaseReservedFrozenAccountTransferFlow record);
}