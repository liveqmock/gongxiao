...
public interface YhglobalReceivedPrepaidAccountMapper extends BaseMapper {
    ...
    @Insert({
            "insert into ${prefix}_yhglobal_received_prepaid_account (rowId, projectId, ",
    ...
    int insert(@Param("prefix") String prefix,
               @Param("record") YhglobalReceivedPrepaidAccount record);

    @Select({
            "select",
            "rowId, projectId, projectName, supplierId, supplierName, status, totalAmount, ",
            "dataVersion, createTime, lastUpdateTime, tracelog",
            "from ${prefix}_yhglobal_received_prepaid_account",
            "where projectId = #{projectId}"
    })
    YhglobalReceivedPrepaidAccount getAccount(@Param("prefix") String prefix,
                                              @Param("projectId") long projectId);

    @Update({
            "update ${prefix}_yhglobal_received_prepaid_account",
