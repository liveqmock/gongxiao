...
public class SupplierAccountServiceImpl extends com.yhglobal.gongxiao.payment.microservice.SupplierAccountServiceGrpc.SupplierAccountServiceImplBase {
    ...
    public void getSupplierCouponBufferToDistributor(SupplierAccountServiceStructure.GetSupplierCouponBufferToDistributorRequest request, StreamObserver<PaymentCommonGrpc.AccountAmountResponse> responseObserver) {
        ...
        try {
            ...
            accountAmount.setCouponAmountDouble(couponBufferToDistributor.getTotalAmount().doubleValue());
...
//            accountAmount.setCouponAmountDouble(roundUpValue);
            ...
            accountAmount.setPrepaidAmountDouble(prepaidBufferToDistributor.getTotalAmount().doubleValue());
...
//            accountAmount.setPrepaidAmountDouble(roundUpValue);
    ...
    public void getSupplierAccountAmount(SupplierAccountServiceStructure.GetSupplierAccountAmountRequest request, StreamObserver<PaymentCommonGrpc.AccountAmountResponse> responseObserver) {
        ...
        try {
            ...
            accountAmount.setCouponAmountDouble(couponAccount!=null?couponAccount.getTotalAmount().doubleValue():0);
...
//            accountAmount.setCouponAmountDouble(roundUpValue);
            ...
            accountAmount.setPrepaidAmountDouble(prepaidAccount!=null?prepaidAccount.getTotalAmount().doubleValue():0);
...
//            accountAmount.setPrepaidAmountDouble(roundUpValue);
    ...
    public void getSellHighAccount(SupplierAccountServiceStructure.GetSellHighAccountRequest request, StreamObserver<SupplierAccountServiceStructure.GetSellHighAccountResponse> responseObserver) {
        ...
        try {
            ...
            SupplierSellHeightAccount account = null;//supplierSellHeightTransferAccountDao.getAccount(prefix, projectId);
    ...
    @Override
    public void writeOffUpdateSellHighAccount(PaymentCommonGrpc.WriteOffRequest request, StreamObserver<PaymentCommonGrpc.WriteOffResponse> responseObserver) {
        ...
        try {
            ...
            if (amount > account.getTotalAmount()) {
                ...
                response = builder.setReturnCode(SELL_HIGH_ACCOUNT_BALANCE_NOT_ENOUGH.getErrorCode()).build();
                responseObserver.onNext(response);
                responseObserver.onCompleted();
                return;
            }
            WriteOffReturnObject writeOffReturnObject = null;//supplierSellHeightTransferAccountService.updateSupplierSellHeightTransferAccount(prefix, rpcHeader, "CNY", projectId, amount, writeOffFlowNo, new Date(transactionTime));
...
//            }
    ...
    @Override
    public void salesUpdateSellHighAccount(SupplierAccountServiceStructure.SalesUpdateSellHighAccountRequest request, StreamObserver<GongxiaoRpc.RpcResult> responseObserver) {
        GongxiaoRpc.RpcResult response;
        //拆分请求参数
        GongxiaoRpc.RpcHeader rpcHeader = request.getRpcHeader();
        String currencyCode = request.getCurrencyCode();
        ...
        try {
            ...
            WriteOffReturnObject writeOffReturnObject = null; //supplierSellHeightTransferAccountService.updateSupplierSellHeightTransferAccount(prefix, rpcHeader, currencyCode, projectId, amount, salesOrderNo, new Date(transactionTime));
            int returnCode = writeOffReturnObject.getReturnCode();
            if (returnCode == 0) {
                String flowNo = writeOffReturnObject.getFlowNo();
                logger.info("#traceId={}# [OUT] update sell high account success. salesOrderNo={}, flowNo={}", rpcHeader.getTraceId(), salesOrderNo, flowNo);
                response = GrpcCommonUtil.success();
                responseObserver.onNext(response);
                responseObserver.onCompleted();
            } else {
                logger.info("#traceId={}# [OUT] update sell high account FAILED. salesOrderNo={}", rpcHeader.getTraceId(), salesOrderNo);
                response = GrpcCommonUtil.fail(ErrorCode.getErrorCodeByCode(returnCode));
                responseObserver.onNext(response);
                responseObserver.onCompleted();
            }
        } catch (Exception e) {
    ...
    @Override
    public void selectSupplierSellHighRecordList(SupplierAccountServiceStructure.SelectSupplierAccountFlowListRequest request, StreamObserver<PaymentCommonGrpc.FrontPageFlowPageInfo> responseObserver) {
        PaymentCommonGrpc.FrontPageFlowPageInfo.Builder builder = PaymentCommonGrpc.FrontPageFlowPageInfo.newBuilder();
        PaymentCommonGrpc.FrontPageFlowPageInfo response;
        //拆分请求参数
        GongxiaoRpc.RpcHeader rpcHeader = request.getRpcHeader();
        ...
        try {
            ...
            PageHelper.startPage(pageNum, pageSize);
            List<SupplierSellHeightTransferRecord> list = null;//supplierSellHeightTransferRecordDao.selectSupplierSellHighRecordList(prefix, currencyCode, projectId, moneyFlow, beginDate, endDate);
            ...
            for (SupplierSellHeightTransferRecord record : list) {
                ...
                pageFlow.setCreateTime(record.getCreateTime());
                pageFlow.setCreateByName(GrpcCommonUtil.getProtoParam(record.getCreatedByName()));
                frontPageFlows.add(pageFlow);
            }
            PageInfo<FrontPageFlow> pageInfo = new PageInfo<>(frontPageFlows);
            pageInfo.setTotal(oldPageInfo.getTotal());
            logger.info("#traceId={}# [OUT]: select record list success.", rpcHeader.getTraceId());
            transFrontPageFlow(pageInfo, builder);
            response = builder.build();
            responseObserver.onNext(response);
            responseObserver.onCompleted();
    ...
    public void getSellHighSubtotal(SupplierAccountServiceStructure.GetSupplierAccountSubtotalRequest request, StreamObserver<PaymentCommonGrpc.FrontPageFlowSubtotal> responseObserver) {
        ...
        try {
            ...
            List<FlowSubtotal> flowSubtotals = null;//supplierSellHeightTransferRecordDao.selectIncomeAndExpenditure(prefix, currencyCode, projectId, moneyFlow, beginDate, endDate);
            FrontPageFlowSubtotal subtotal = AccountSubtotalUtil.getSubtotal(flowSubtotals);
    ...
    public void selectIncomeAndExpenditure(SupplierAccountServiceStructure.SelectIncomeAndExpenditureRequest request, StreamObserver<PaymentCommonGrpc.FrontPageFlowSubtotal> responseObserver) {
        ...
        try {
            ...
            for (FlowSubtotal flowSubtotal : flowSubtotals) {
                if (YhGlobalInoutFlowConstant.FLOW_TYPE_IN.getNum() == flowSubtotal.getFlowType()) {
                    frontPageFlowSubtotal.setIncomeQuantity(flowSubtotal.getCount());
                    frontPageFlowSubtotal.setIncomeAmount(flowSubtotal.getAmountCount());
                } else if (YhGlobalInoutFlowConstant.FLOW_TYPE_OUT.getNum() == flowSubtotal.getFlowType()) {
                    frontPageFlowSubtotal.setExpenditureQuantity(flowSubtotal.getCount());
                    frontPageFlowSubtotal.setExpenditureAmount(flowSubtotal.getAmountCount());
                }
            }
    ...
    public void updateSupplierSellHighAccountOnJmgo(SupplierAccountServiceStructure.UpdateSupplierSellHighAccountOnJmgoRequest request, StreamObserver<GongxiaoRpc.RpcResult> responseObserver) {
        ...
        try {
            ...
            while (maxRetryTimes-- > 0) {
                ...
                //更新账户
                int update = 1;//supplierSellHeightTransferAccountDao.update(prefix, account);
                //插入记录
//                if (update == 1) {
//                    // 更新成功，插入流水,跳出循环
