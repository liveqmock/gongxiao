...
import com.yhglobal.gongxiao.phjd.task.SyncPlanInboundToWmsTask;
import com.yhglobal.gongxiao.phjd.task.SyncPurchaseOrderToEasTask;
    ...
    public void getPurchaseOrderList(PurchaseStructure.GetPurchaseOrderListReq request,
        ...
        try {
            ...
            for (PurchaseOrder purchaseOrder : purchaseOrderList) {
                ...
                if (orderStatus == PurchaseStatus.ORDER_IMPORT.getStatus()) {
                    purchaseOrderStatus = "草稿";
                } else if (orderStatus == PurchaseStatus.ORDER_SUBMIT.getStatus()) {
                    purchaseOrderStatus = "待发货";
                } else if (orderStatus == PurchaseStatus.ORDER_CUT_PAYMENT.getStatus()) {
                    purchaseOrderStatus = "待发货";
                } else if (orderStatus == PurchaseStatus.ALREADY_PLAN.getStatus()) {
                    purchaseOrderStatus = "待收货";
                } else if (orderStatus == PurchaseStatus.PART_INBOUND.getStatus()) {
                    purchaseOrderStatus = "部分收货";
                } else if (orderStatus >= PurchaseStatus.ORDER_COMPLETE.getStatus()
                        && orderStatus < PurchaseStatus.CANCEL.getStatus()) {
                    purchaseOrderStatus = "交易成功";
                } else if (orderStatus == PurchaseStatus.CANCEL.getStatus()) {
                ...
                //显示作废
                if (orderStatus == PurchaseStatus.ALREADY_PLAN.getStatus()
                        || orderStatus == PurchaseStatus.ORDER_IMPORT.getStatus()
                        || orderStatus == PurchaseStatus.ORDER_SUBMIT.getStatus()
                ) {
                    enableCancle = true;
                }
                boolean enablePlanInstrock = false;
                //显示预约入库
                if (orderStatus == PurchaseStatus.ORDER_CUT_PAYMENT.getStatus()
                        || orderStatus == PurchaseStatus.ALREADY_PLAN.getStatus()
                        || orderStatus == PurchaseStatus.PART_INBOUND.getStatus()) {
                ...
                if (orderStatus == PurchaseStatus.ALREADY_PLAN.getStatus()
                        || orderStatus == PurchaseStatus.PART_INBOUND.getStatus()
                        || orderStatus == PurchaseStatus.ORDER_COMPLETE.getStatus()
                        || orderStatus == PurchaseStatus.EXCEPTION_COMPLETE_HANDLED.getStatus()
                        || orderStatus == PurchaseStatus.EXCEPTION_COMPLETE_UNHANDLED.getStatus()
                        || orderStatus == PurchaseStatus.GENERATE_COUPON.getStatus()) {
                    enableDetail = true;
                }
