...
public enum GrossProfitTransferPatternConstant {
    ...
    private volatile static Map<String, GrossProfitTransferPatternConstant> mapping1 = null;

    public static GrossProfitTransferPatternConstant getGrossProfitTransferPatternConstantByName(String accountName){
        if (mapping1 == null) {
            synchronized (GrossProfitTransferPatternConstant.class) {
                if (mapping1 == null) {
                    mapping1 = new HashMap<>();
                    for (GrossProfitTransferPatternConstant grossProfitTransferPatternConstan : GrossProfitTransferPatternConstant.values()) {
                        mapping1.put(grossProfitTransferPatternConstan.AccountName, grossProfitTransferPatternConstan);
                    }
                }
            }
        }
