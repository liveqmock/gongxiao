...
public interface SupplierCouponBufferToYhglobalMapper extends BaseMapper {
    ...
    @Insert({
            "insert into ${prefix}_supplier_coupon_buffer_to_yhglobal (supplierId, supplierName, ",
            "projectId, ",
            "projectName, status, ",
    ...
    int insert(@Param("prefix") String prefix,
               @Param("record")SupplierCouponBufferToYhglobal record);

    @Select({
            "select",
            "supplierId, supplierName, projectId, projectName, status, totalAmount, ",
            "dataVersion, createTime, lastUpdateTime, tracelog",
            "from ${prefix}_supplier_coupon_buffer_to_yhglobal",
            "where  projectId = #{projectId}"
    })
    SupplierCouponBufferToYhglobal getAccount(@Param("prefix") String prefix,
                                              @Param("projectId") long projectId);


    @Update({
            "update ${prefix}_supplier_coupon_buffer_to_yhglobal",
