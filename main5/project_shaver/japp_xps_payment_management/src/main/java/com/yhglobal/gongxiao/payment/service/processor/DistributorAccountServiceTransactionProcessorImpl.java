...
public class DistributorAccountServiceTransactionProcessorImpl implements DistributorAccountServiceTransactionProcessor {
    ...
    public RpcResult<CashCouponPrepaidFlows> payForSalesOrder(String prefix, GongxiaoRpc.RpcHeader rpcHeader, String currencyCode, long distributorId, long projectId, long couponAmount, long prepaidAmount, long cashAmount, String salesOrderNo, Date transactionTime) {
        ...
        try {
            ...
            if (cashAmount > 0) {
                try {
                    //前往现金账账户扣款，返回流水号
                    String couponFlowNo = distributorCashAccountService.updateDistributorCashAccount(prefix, rpcHeader,
            ...
            if (couponAmount > 0) {
                try {
                    //前往返利账户扣款，返回流水号
                    String couponFlowNo = distributorCouponAccountService.updateDistributorCouponAccount(prefix, rpcHeader,
                            currencyCode, distributorId,
            ...
            if (prepaidAmount > 0) {
                try {
                    //前往返利账户扣款，返回流水号
                    String prepaidFlow = distributorPrepaidAccountService.updateDistributorPrepaidAccount(prefix, rpcHeader,
                            currencyCode, distributorId,
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public RpcResult<CashCouponPrepaidFlows> returnForSalesReturnOrder(String prefix, GongxiaoRpc.RpcHeader rpcHeader, String currencyCode, long distributorId, long projectId, long couponAmount, long prepaidAmount, long cashAmount, String salesReturnOrderNo, Date transactionTime) {
        ...
        try {
            ...
            if (cashAmount > 0) {
                //现金转入，返回流水号
                String cashFlowNo = distributorCashAccountService.updateDistributorCashAccount(prefix, rpcHeader,
                        currencyCode, distributorId,
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public RpcResult<String> depositCouponReceived(String prefix,
                                                   GongxiaoRpc.RpcHeader rpcHeader, String currencyCode, long distributorId, String distributorName, long projectId, long couponAmount, Date transactionTime, String remark) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public RpcResult<String> depositPrepaidReceived(String prefix, GongxiaoRpc.RpcHeader rpcHeader, String currencyCode, long distributorId, String distributorName, long projectId, long prepaidAmount, Date transactionTime, String remark) {
        ...
        try {
            ...
            String flowNo = supplierPrepaidBufferToDistributorService.updatePrepaidBufferToDistributorAccount(prefix, rpcHeader,
                    currencyCode, projectId, distributorId, distributorName, -prepaidAmount, null, transactionTime, remark, null);
            //返利账户转入
            String couponFlowNo = distributorPrepaidAccountService.updateDistributorPrepaidAccount(prefix, rpcHeader,
                    currencyCode, distributorId,
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public RpcResult depositCouponReceivedAccounts(String prefix, GongxiaoRpc.RpcHeader rpcHeader, List<DistributorCouponAccount> distributorCouponAccounts) {
        ...
        try {
            for (DistributorCouponAccount pageAccount : distributorCouponAccounts) {
                DistributorCouponAccount databaseAccount = distributorCouponAccountDao.getAccount(prefix, pageAccount.getProjectId(), pageAccount.getDistributorId());
                //每个账户转入的金额
                double transferAmountDouble = pageAccount.getTransferAmountDouble();
                long transferAmount = (long) (transferAmountDouble * HUNDRED);
                RpcResult<String> rpcResult = this.depositCouponReceived(prefix, rpcHeader, databaseAccount.getCurrencyCode(),
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public RpcResult depositPrepaidReceivedAccounts(String prefix,
        ...
        try {
            for (DistributorPrepaidAccount pageAccount : distributorPrepaidAccounts) {
                DistributorPrepaidAccount databaseAccount = distributorPrepaidAccountDao.getAccount(prefix, pageAccount.getProjectId(), pageAccount.getDistributorId());
                //每个账户转入的金额
                double transferAmountDouble = pageAccount.getTransferAmountDouble();
                long transferAmount = (long) (transferAmountDouble * HUNDRED);
                RpcResult<String> rpcResult = this.depositPrepaidReceived(prefix, rpcHeader, databaseAccount.getCurrencyCode(),
                        databaseAccount.getDistributorId(), databaseAccount.getDistributorName(),
                        databaseAccount.getProjectId(), transferAmount,
                        new Date(), null);
                if (!rpcResult.getSuccess()) {
