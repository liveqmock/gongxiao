...
public class SalesOrderServiceImpl extends SalesOrderServiceGrpc.SalesOrderServiceImplBase {
    ...
    public void getOrderByOrderNo(SalesOrderServiceStructure.CommonSalesOrderRequest request, StreamObserver<SalesOrderServiceStructure.GetOrderByOrderNoResponse> responseObserver) {
        ...
        try {
            ...
            DistributorAccountServiceStructure.GetDistributorAccountAmountRequest distributorAccountAmountRequest =
                    accountBuilder.setProjectId(salesOrder.getProjectId()).
                            setCurrencyCode(salesOrder.getCurrencyCode()).
                            setDistributorId(salesOrder.getDistributorId()).build();
            PaymentCommonGrpc.AccountAmountResponse distributorAccountAmount = accountStub.getDistributorAccountAmount(distributorAccountAmountRequest);
            long accountCouponAmount = distributorAccountAmount.getCouponAmount();
            String accountCouponAmountDouble = DoubleScale.keepTwoBits(accountCouponAmount);
            long accountPrepaidAmount = distributorAccountAmount.getPrepaidAmount();
            String accountPrepaidAmountDouble = DoubleScale.keepTwoBits(accountPrepaidAmount);
            // 订单需付金额 = 订单总金额 - 现金
            long totalToPay = salesOrder.getTotalOrderAmount() - salesOrder.getPrestoredAmount();
            String cashAmountDouble = DoubleScale.keepTwoBits(totalToPay);
            ...
            respBuilder.setSalesOrderNo(salesOrderNo)
                    ...
                    .setCashAmountDouble(cashAmountDouble)
                    ...
                    .setDistributorCouponAmountDouble(accountCouponAmountDouble)
                    .setDistributorPrepaidAmountDouble(accountPrepaidAmountDouble);
        ...
        try {
            ...
            for (SalesOrderServiceStructure.SalesOrderItemInfoRequest info : requestList) {
                ...
                if (grossProfitValue == 0) {
                    ...
                    if (difference > 0) {
                        //产生差价
                        totalSellHighAmount += (difference * quantity / 10000.0);
                    ...
                    // 应收毛利(销售指导价-采购指导价)*数量
                    long shouldReceiveGrossProfit = salesGuidePrice - purchaseGuidePrice;
                    salesOrderItem.setShouldReceiveGrossProfit(shouldReceiveGrossProfit);
                    totalShouldReceiveGrossProfit += (shouldReceiveGrossProfit * quantity / 10000.0);
                    // 出货价 - 销售指导价 > 0 ,差值为销售差价, 实收毛利 = 销售指导价 - 采购指导价
                    if (wholesalePrice - salesGuidePrice > 0) {
                        totalSellHighAmount += ((wholesalePrice - salesGuidePrice) * quantity / 10000.0);
                        salesOrderItem.setSellHighAmount(wholesalePrice - salesGuidePrice);
                        totalReceivedGrossProfit += ((salesGuidePrice - purchaseGuidePrice) * quantity / 10000.0);
                        salesOrderItem.setReceivedGrossProfit(salesGuidePrice - purchaseGuidePrice);
                    }
                    // 采购指导价 - 出货价 > 0, 差值为应收代垫, 实收毛利 = 0
                    else if (purchaseGuidePrice - wholesalePrice > 0) {
                        salesOrderItem.setGeneratedPrepaid(purchaseGuidePrice - wholesalePrice);
                        totalGeneratedPrepaid += ((purchaseGuidePrice - wholesalePrice) * quantity / 10000.0);
                        salesOrderItem.setReceivedGrossProfit(0L);
                    }
                    // 采购指导价 < 出货价 < 销售指导价, 实收毛利 = 出货价 - 采购指导价
                    else if (purchaseGuidePrice < wholesalePrice && wholesalePrice < salesGuidePrice) {
                        salesOrderItem.setReceivedGrossProfit(wholesalePrice - purchaseGuidePrice);
                        totalReceivedGrossProfit += ((wholesalePrice - purchaseGuidePrice) * quantity / 10000.0);
                    }
                }
    ...
    public void approveSalesOrder(SalesOrderServiceStructure.ApproveSalesOrderRequest request, StreamObserver<GongxiaoRpc.RpcResult> responseObserver) {
        GongxiaoRpc.RpcHeader rpcHeader = request.getRpcHeader();
        String salesOrderNo = request.getSalesOrderNo();
        double couponAmountDouble = request.getCouponAmountDouble();
        double prepaidAmountDouble = request.getPrepaidAmountDouble();
        ...
        try {
            logger.info("#traceId={}# [IN][approveSalesOrder] params: salesOrderNo={}, couponAmountDouble={}, prepaidAmountDouble={}, cashAmountDouble={}",
                    rpcHeader.getTraceId(), salesOrderNo, couponAmountDouble, prepaidAmountDouble);
            // 0.查询表前缀
            String prefix = TablePrefixUtil.getTablePrefixByProjectId(rpcHeader, projectId);
...
//            一、查找到订单
            ...
            while (maxRetryTimes-- > 0) {
                ...
                totalAmount = salesOrder.getTotalOrderAmount() - couponAmount - prepaidAmount - salesOrder.getPrestoredAmount();

                if (couponAmount < 0 || prepaidAmount < 0 || totalAmount < 0) {
                    logger.info("#traceId={}# [OUT]: can NOT approve sales order. salesOrderNo={}, couponAmount={}, prepaidAmount={}, totalPaid={}", rpcHeader.getTraceId(), salesOrderNo, couponAmount, prepaidAmount, totalAmount);
                ...
                DistributorAccountServiceStructure.GetDistributorAccountAmountRequest.Builder accountBuilder = DistributorAccountServiceStructure.GetDistributorAccountAmountRequest.newBuilder();
                DistributorAccountServiceStructure.GetDistributorAccountAmountRequest distributorAccountAmountRequest =
                        accountBuilder.setProjectId(projectId).setCurrencyCode(salesOrder.getCurrencyCode()).setDistributorId(salesOrder.getDistributorId()).build();
                PaymentCommonGrpc.AccountAmountResponse account = accountStub.getDistributorAccountAmount(distributorAccountAmountRequest);
                if (account.getCouponAmount() < couponAmount) {
                    //返利余额不足
                    logger.info("#traceId={}# [OUT]: can NOT approve sales order. salesOrderNo={}", rpcHeader.getTraceId(), salesOrderNo);
                    response = GrpcCommonUtil.fail(DISTRIBUTOR_COUPON_ACCOUNT_BALANCE_NOT_ENOUGH.getErrorCode(), DISTRIBUTOR_COUPON_ACCOUNT_BALANCE_NOT_ENOUGH.getMessage());
                    responseObserver.onNext(response);
                    responseObserver.onCompleted();
                    return;
                }
                if (account.getPrepaidAmount() < prepaidAmount) {
                    //代垫余额不足
                    logger.info("#traceId={}# [OUT]: can NOT approve sales order. salesOrderNo={}", rpcHeader.getTraceId(), salesOrderNo);
                    response = GrpcCommonUtil.fail(DISTRIBUTOR_PREPAID_ACCOUNT_BALANCE_NOT_ENOUGH.getErrorCode(), DISTRIBUTOR_PREPAID_ACCOUNT_BALANCE_NOT_ENOUGH.getMessage());
                    responseObserver.onNext(response);
                    responseObserver.onCompleted();
                    return;
                }
                ...
                update = salesOrderDao.updateApprovalInfo(prefix, salesOrder.getSalesOrderId(), salesOrder.getDataVersion(), salesOrderStatus, couponAmount, prepaidAmount, totalAmount, new Date(), traceLogs, salesContractNo);
                if (update == 1) {
                    updateSuccess = true;
                    break;
                }
            ...
            if (couponAmount > 0 || prepaidAmount > 0) {
                ...
                if (payResponse.getReturnCode() == 0) {
                    String couponFlowNo = payResponse.getResult().getCouponFlowNo();
                    String prepaidFlowNo = payResponse.getResult().getPrepaidFlowNo();
                    update = salesOrderDao.updateCouponPrepaidFlowNo(prefix, salesOrder.getSalesOrderId(), salesOrder.getDataVersion(), couponFlowNo, prepaidFlowNo);
                } else {
            ...
            for (SalesOrderItem salesOrderItem : salesOrderItems) {
                ...
                long usedCouponAmount = couponAmount * salesOrderItem.getTotalOrderAmount() / totalOrderAmount;
                // 使用代垫金额 = 代垫使用总金额*商品小计金额/订单总金额
                long userPrepaidAmount = prepaidAmount * salesOrderItem.getTotalOrderAmount() / totalOrderAmount;
                update = salesOrderItemDao.updateCouponPrepaidAmount(prefix, salesOrderItem.getSalesOrderItemId(), salesOrderItem.getDataVersion(), usedCouponAmount, userPrepaidAmount);
                if (update != 1) {
                    logger.error("FAILED to approve salesOrder item. salesOrderNo={}, productCode={},couponAmount={}, prepaidAmount={}", salesOrderNo, salesOrderItem.getProductCode(), usedCouponAmount, userPrepaidAmount);
                }
            }
    ...
    public void cancelSalesOrderApprove(SalesOrderServiceStructure.CommonSalesOrderRequest request, StreamObserver<GongxiaoRpc.RpcResult> responseObserver) {
        ...
        try {
            ...
            DistributorAccountServiceStructure.PayForSalesOrderRequest.Builder payBuilder = DistributorAccountServiceStructure.PayForSalesOrderRequest.newBuilder()
                    ...
                    .setDistributorId(salesOrder.getDistributorId())
                    .setProjectId(projectId)
                    .setCouponAmount(salesOrder.getCouponAmount())
                    .setPrepaidAmount(salesOrder.getPrepaidAmount())
                    .setCashAmount(salesOrder.getPrestoredAmount())
                    .setSalesOrderNo(salesOrderNo)
                    .setTransactionTime(System.currentTimeMillis());
            DistributorAccountServiceStructure.PayForSalesOrderRequest payRequest = payBuilder.build();
            ...
            if (payResponse.getReturnCode() == 0) {
                String returnCashFlowNo = payResponse.getResult().getCashFlowNo();
                String returnCouponFlowNo = payResponse.getResult().getCouponFlowNo();
                String returnPrepaidFlowNo = payResponse.getResult().getPrepaidFlowNo();
                int i = salesOrderDao.updateReturnFlowNos(prefix, salesOrder.getSalesOrderId(), salesOrder.getDataVersion(), returnCashFlowNo, returnCouponFlowNo, returnPrepaidFlowNo);
            } else {
                logger.error("FAILED to return amount. salesOrderNo={}, cashAmount={}, couponAmount={}, prepaidAmount={}", salesOrderNo, salesOrder.getCashAmount(), salesOrder.getCouponAmount(), salesOrder.getPrepaidAmount());
            }
            //作废现金确认
            ...
            for (SalesOrderItem salesOrderItem : salesOrderItems) {
                String productCode = salesOrderItem.getProductCode();
                Integer quantity = salesOrderItem.getTotalQuantity();
                RpcResult rpcResult = planSaleItemService.cancelAuditSaleOrder(prefix, rpcHeader, projectId + "", distributorId + "", productCode, createTime, quantity);
                if (!rpcResult.getSuccess()) {
                    logger.error("FAILED to update planItem quantity. productCode={}", productCode);
                }
            }
