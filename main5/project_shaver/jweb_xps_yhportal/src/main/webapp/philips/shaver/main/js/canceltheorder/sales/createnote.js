    ...
    function init() {
                ...
                if (res.returnCode === 0) {
                    ...
                    $.each(maskLoadRadioList, function (index, item) {
                        ...
                        tempRadio += '<p><input type="radio" id="' + item.gongxiaoOutboundOrderNo + '" name="product">' +
                            ...
                            '</strong>' + '<span>状态：</span>' + '<strong>' + item.orderStatusStr +
        ...
        $createnoteSubmit.click(function () {
            ...
            $.each(yhTableData,function (index,returnProItem) {
                if(returnProItem["returnNumber"] > returnProItem["canReturnQuantity"]){
                    return false;
                }
            });
            ...
            columns: [{
                ...
                events: {
                    'blur .return-number': function (e, value, row, index) {
                        ...
                        var _canReturnQuantity = row['canReturnQuantity'];
                        if(inputValue > _canReturnQuantity || inputValue == 0){
