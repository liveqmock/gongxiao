...
public class SyncCouponPrepaidToYhglobalReceivedTask implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (couponAmount != 0) {
                //缓冲账户扣款
                String couponFlowNo = supplierCouponBufferToYhglobalService.updateCouponBufferToYhglobal(prefix, rpcHeader, currencyCode, projectId, -couponAmount, purchaseOrderNo, transactionTime, null, null);
                //实收账户添加
                yhglobalReceivedCouponAccountService.updateYhglobalReceivedCouponAccount(prefix, rpcHeader, currencyCode, projectId, couponAmount, transactionTime, purchaseOrderNo, couponFlowNo);
            }

            if (prepaidAmount != 0) {
                //缓冲账户扣款
                String prepaidFlowNo = supplierPrepaidBufferToYhglobalService.updatePrepaidBufferToYhglobal(prefix, rpcHeader, currencyCode, projectId, -prepaidAmount, purchaseOrderNo, transactionTime, null, null);
                //实收账户添加
                yhglobalReceivedPrepaidAccountService.updateYhglobalReceivedPrepaidAccount(prefix, rpcHeader, currencyCode, projectId, prepaidAmount, transactionTime, purchaseOrderNo, prepaidFlowNo);
            }
