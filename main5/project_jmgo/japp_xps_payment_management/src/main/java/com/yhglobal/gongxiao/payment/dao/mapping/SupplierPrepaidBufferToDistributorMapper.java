...
public interface SupplierPrepaidBufferToDistributorMapper extends BaseMapper {
    ...
    @Insert({
            "insert into ${prefix}_supplier_prepaid_buffer_to_distributor (supplierId, supplierName, ",
            "projectId, ",
            "projectName, status, ",
    ...
    int insert(@Param("prefix") String prefix,
               @Param("record") SupplierPrepaidBufferToDistributor record);

    @Select({
            "select",
            "supplierId, supplierName, projectId, projectName, status, totalAmount, ",
            "dataVersion, createTime, lastUpdateTime, tracelog",
            "from ${prefix}_supplier_prepaid_buffer_to_distributor",
            "where projectId = #{projectId}"
    })
    SupplierPrepaidBufferToDistributor getAccount(@Param("prefix") String prefix,
                                                  @Param("projectId") long projectId);


    @Update({
            "update ${prefix}_supplier_prepaid_buffer_to_distributor",
