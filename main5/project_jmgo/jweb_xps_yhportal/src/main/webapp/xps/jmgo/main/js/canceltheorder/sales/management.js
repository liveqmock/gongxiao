        ...
        $yhTable.bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function (value, row) {
                    if(row['returnedOrderStatus'] === 1){
                        return '<a class="cancel-review btn btn-primary">审核</a>'
                    }else{
                        return ""
                    }
                },
                events: {
                    'click .cancel-review': function (event, value, row) {
                        layui.use('layer',function () {
                            var layer = layui.layer;
                            layer.alert("您确认作审核订单吗？",{
                                icon: 0
                            },function () {
                                var cancelParams = {
                                    salesReturnedOrderId: row.salesReturnedOrderId,
            ...
            onCheck:function() {
                $verify.removeClass("yh-handle-btn").removeAttr("disabled");
            },
            onUncheck:function(){
                var tableSelectedData = $yhTable.bootstrapTable("getSelections");
                if(tableSelectedData.length === 0){
                    $verify.addClass("yh-handle-btn").attr("disabled",true);
                }
            },
