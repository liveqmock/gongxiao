$(function () {
    ...
    function renderSelectList(URL) {
        $.getSelectOptions("projectlist", URL + "/project/getProjectList", "projectId", "projectName")
            .then(function () {
                $.get(URL+"/payment/supplier/reserve/sellHighAccount",{
                    projectId: $("#projectlist").val()
                },function(res){
                    if(res.returnCode === 0){
                        console.log(res['data']['totalAmountDoubleStr'])
                        var totalAmountDoubleStr = res['data']['totalAmountDoubleStr'];
                        $("#price-differences").text(totalAmountDoubleStr)
                    }
                },'json')
            })
    }
        ...
        $.get(URL + "/sales/getOrderInfo", {
            ...
            if (res.returnCode === 0) {
                ...
                var $salesContractNo = $("#sales-contractno");
                renderTable();
                renderLogTable();
                // 销售单商品列表
                ...
                $salesContractNo.val(orderData['salesContractNo']);
                ...
                var amountInfo = [orderData['supplierDiscountAmountDouble'],orderData['yhDiscountAmountDouble']];
                ...
                var amountmoney = orderData['cashAmountDouble'] || 0
                // 出货总金额
        ...
        $("#couponamount").on("input propertychange",function () {
            ...
            var 0 = (Number(_shipmentAmount) - prepaidAmount - cashAmount - inputValue).toFixed(2);
        ...
        */
        $("#price-differences-use").on('input propertychange',function(){
            var _input = $(this).val();
            var priceDifferences = $("#price-differences").html();
            var surplusValue = 0;
            if(_input > 0 && priceDifferences - _input > 0 && _input){
                surplusValue = _shipmentAmount - _input - _cashAmount;
            }else{
                surplusValue = _shipmentAmount - _cashAmount;
            }
            $("#amountmoney").val(surplusValue);
        })
        ...
        function reviewSkip() {
            ...
            $reviewBtn.click(function () {
                var priceDifferencesUse = $("#price-differences-use").val();
                var priceDifferences = $("#price-differences").text()
                var $salesContractNo = $("#sales-contractno");
                // 应收金额
                ...
                var reviewParams = {
                    salesOrderNo: orderNumber,
                    // 差价账户使用金额
                    discountAmountDouble:priceDifferencesUse,
                    // 销售合同号
                    salesContractNo:$salesContractNo.val()
                };
                if(priceDifferencesUse - priceDifferences < 0 && (_shipmentAmount - _cashAmount - priceDifferencesUse - amountMoney) > 0){
