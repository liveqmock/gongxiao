...
import com.yhglobal.gongxiao.payment.microservice.YhglobalPrepaidServiceGrpc;
import com.yhglobal.gongxiao.payment.microservice.YhglobalPrepaidServiceStructure;
            ...
            for (String outboundOrderNo : outboundMap.keySet()) {
                ...
                for (OutboundOrderItem item : outboundItems) {
                    ...
                    salesOutboundOrderItemDao.insert(prefix, productCode, outboundOrderNo, salesOrderNo,
                            ...
                            salesPlanItem.getYhPrepaidDiscount(), sellHighAmount, shouldReceivedPrepaid, shouldReceivedGrossProfit, receivedGrossProfit, batchNo);
                }
            }
    ...
    public void outboundSigned(DeliverScheduleStructure.OutboundSignedRequest request, StreamObserver<GongxiaoRpc.RpcResult> responseObserver) {
        ...
        try {
            ...
            while (maxRetryTimes-- > 0) {
                ...
                Date signed = new Date(signedTime);
            ...
            if (sellHighAmount > 0) {
                ...
                if (ErrorCode.SUCCESS != ErrorCode.getErrorCodeByCode(rpcResult.getReturnCode())) {
                    // 如果失败
                    logger.error("sales order update coupon/prepaid FAILED! salesOrderNo={}, amount={}", salesOrderNo, sellHighAmount);
                }
            }
