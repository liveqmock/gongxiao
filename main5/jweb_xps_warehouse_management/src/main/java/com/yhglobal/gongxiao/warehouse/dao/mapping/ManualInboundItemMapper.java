...
public interface ManualInboundItemMapper extends BaseMapper {
    @Insert({
            "<script>",
            "insert into warehouse_manual_inbound_order_item (",
    ...
    @Update({
            "update warehouse_manual_inbound_order_item",
    ...
    @Select({
            ...
            "guidePrice, purchasePrice, costPrice, ",
            "note, status, createTime",
            "from warehouse_manual_inbound_order_item where gongxiaoInboundOrderNo = #{gongxiaoInboundOrderNo} and productCode = #{productCode}"
    })
