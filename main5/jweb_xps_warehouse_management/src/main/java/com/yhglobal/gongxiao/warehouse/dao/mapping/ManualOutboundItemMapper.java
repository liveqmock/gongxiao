...
public interface ManualOutboundItemMapper extends BaseMapper{
    ...
    @Insert({
            "<script>",
            "insert into warehouse_manual_outbound_order_item (",
    ...
    @Update({
            "update warehouse_manual_outbound_order_item",
            "set ",
    ...
    @Select({
            ...
            "guidePrice, wholesalePriceDouble, ",
            "note, status, createTime",
            "from warehouse_manual_outbound_order_item where gongxiaoOutboundOrderNo = #{gongxiaoOutboundOrderNo} and productCode = #{productCode}"
    })
